{
	"translatorID": "0ad181a5-a053-491d-a28a-addccaab3902",
	"label": "Scholars Portal Journals",
	"creator": "Taylor Shek-wai Hui",
	"target": "^http://journals\\d\\.scholarsportal\\.info/",
	"minVersion": "1.1",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
	"translatorType": 4,
	"browserSupport": "gcsb",
	"lastUpdated": "2014-01-20 11:57:03"
}
...
/**
	Copyright (c) 2014, Taylor Shek-wai Hui
...
// Zotero Translator for Scholars Portal Journals
// version 1.0
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/(journals\d\.scholarsportal\.info\/search)/),
choices : {
 title          : FW.Xpath('//li[@class="journal-item"]//*[@class="article-title"]').text().trim().replace(/\n/g," ").capitalizeTitle(),
 urls      		: FW.Xpath('//li[@class="journal-item"]//h3/a').key('href').text()
}
});
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/(journals\d\.scholarsportal\.info\/browse)/),
choices : {
 title          : FW.Xpath('//li[@class="journal-item"]//*[@class="article-title"]').text().trim().replace(/\n/g," ").capitalizeTitle(),
 urls      		: FW.Xpath('//li[@class="journal-item"]//h4/a').key('href').text()
}
});
...
detect           : FW.Url().match(/(journals\d\.scholarsportal\.info\/details)/),
title            : FW.Xpath('//span[@class="article-title"]').text().trim().replace(/\n/g," ").capitalizeTitle(),
publicationTitle : FW.Xpath('//div[@class="journal-title"]/a/span').text().trim(),
volume           : FW.Xpath('//div[@class="journal-info"]').text().match(/Volume(\d+)/).match(/(\d+)/),
issue            : FW.Xpath('//div[@class="journal-info"]').text().match(/Issue(\d+)/).match(/(\d+)/),
pages            : FW.Xpath('//div[@class="journal-info"]').text().match(/(p\.)(\d+)(To-\d+)?/).replace(/p\./,"").replace(/To/,""),
creators         : FW.Xpath('//div[@id="authors"]/span/ul/li').text().trim().cleanAuthor(),
abstractNote 	 : FW.Xpath('//div[@class="journal-abstract"]').text().trim().replace(/\n/g," "),
date 		     : FW.Xpath('//div[@class="journal-info"]').text().match(/(Source Information)(.+)(\d+,)/).match(/\d+/),
DOI				 : FW.Xpath('//article/a[1]').text().trim().replace(/doi:/,""),
url				 : FW.Xpath('//input[@class="span6"]').key('value').text(),
tags             : FW.Xpath('//ul[@class="keywords-list"]/li').text().trim(),
ISSN			 : FW.Url().match(/(details\/[0-9xX]+)/).replace(/details\//,"")
...
var testCases = [
	{
		"type": "web",
		"url": "http://journals1.scholarsportal.info/details/01664972/v26i0010/1127_saiacscomaeo.xml?q=non-profit&search_in=KW&op=AND&q=value+chain&search_in=anywhere&date_from=&date_to=&sort=relevance&sub=",
		"items": [
			{
				"itemType": "journalArticle",
				"creators": [
	...
	{
		"type": "web",
		"url": "http://journals1.scholarsportal.info/browse/01664972",
		"items": "multiple"
	},
	{
		"type": "web",
		"url": "http://journals1.scholarsportal.info/search?q=propensity+score+matching&search_in=anywhere&date_from=&date_to=&sort=relevance&op=AND&search_in=JOURNAL&sub=",
		"items": "multiple"
	},
	{
		"type": "web",
		"url": "http://journals2.scholarsportal.info/details?uri=/00029262/v172i0009/1092_scfstommotps.xml",
		"items": [
			{
				"itemType": "journalArticle",
				"creators": [
